    ...
    @if(isset($movieData['poster_path']) && $movieData['poster_path'])
        <div class="fixed inset-0 z-0 mt-[64px]">
            <div class="absolute inset-0 bg-cover bg-center bg-no-repeat bg-fixed" 
                 style="background-image: url('{{ \App\Helpers\ImageHelper::getPosterUrl($movieData['poster_path']) }}'); filter: blur(10px); transform: scale(1.1); background-size: cover; background-repeat: no-repeat; background-position: center center;"></div>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white bg-opacity-95 backdrop-blur-sm rounded-lg shadow-xl overflow-hidden mb-8">
            <div class="md:flex">
                ...
                <div class="md:w-1/3">
                    @if(isset($movieData['poster_path']) && $movieData['poster_path'])
                        <img src="{{ \App\Helpers\ImageHelper::getPosterUrl($movieData['poster_path']) }}" 
                             alt="{{ $movieData['title'] }}" 
                ...
                <div class="md:w-2/3 p-6">
                    <div class="mb-4">
                        <h1 class="text-4xl font-bold text-gray-900 mb-2">{{ $movieData['title'] }}</h1>
                        @if(isset($movieData['original_title']) && $movieData['original_title'] !== $movieData['title'])
                            <p class="text-xl text-gray-600 italic">{{ $movieData['original_title'] }}</p>
                        @endif
                        @if(isset($movieData['tagline']) && $movieData['tagline'])
                            <p class="text-lg text-gray-700 mt-2 italic">"{{ $movieData['tagline'] }}"</p>
                        @endif
                    </div>

                    
                    @if(isset($movieData['vote_average']) && $movieData['vote_average'])
                        <div class="flex items-center mb-6">
                            <div class="flex items-center">
                                @for($i = 1; $i <= 5; $i++)
                                    @if($i <= ($movieData['vote_average'] / 2))
                            ...
                            <span class="ml-2 text-lg font-semibold text-gray-700">{{ number_format($movieData['vote_average'], 1) }}/10</span>
                            @if(isset($movieData['vote_count']))
                                <span class="ml-2 text-sm text-gray-500">({{ number_format($movieData['vote_count']) }} votes)</span>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        @if(isset($movieData['release_date']))
                            <div>
                                ...
                                <span class="ml-2 text-gray-600">{{ \Carbon\Carbon::parse($movieData['release_date'])->format('d/m/Y') }}</span>
                            </div>
                        @endif
                        @if(isset($movieData['runtime']) && $movieData['runtime'])
                            <div>
                                <span class="text-sm font-semibold text-gray-700">Durée :</span>
                                <span class="ml-2 text-gray-600">{{ $movieData['runtime'] }} minutes</span>
                            </div>
                        @endif
                        @if(isset($movieData['original_language']))
                            <div>
                                <span class="text-sm font-semibold text-gray-700">Langue originale :</span>
                                <span class="ml-2 text-gray-600">{{ strtoupper($movieData['original_language']) }}</span>
                            </div>
                        @endif
                        @if(isset($movieData['status']))
                            <div>
                                <span class="text-sm font-semibold text-gray-700">Statut :</span>
                                <span class="ml-2 text-gray-600">{{ $movieData['status'] }}</span>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        @if(isset($movieData['overview']) && $movieData['overview'])
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="bg-white bg-opacity-95 backdrop-blur-sm rounded-lg shadow-xl p-8">
                ...
                @if(isset($movieData['production_companies']) && is_array($movieData['production_companies']))
                    <div class="mb-6">
                        ...
                        <ul class="space-y-2">
                            @foreach($movieData['production_companies'] as $company)
                ...
                @if(isset($movieData['spoken_languages']) && is_array($movieData['spoken_languages']))
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($movieData['spoken_languages'] as $language)
            ...
            <div class="bg-white bg-opacity-95 backdrop-blur-sm rounded-lg shadow-xl p-8">
                ...
                @if(isset($movieData['budget']) && $movieData['budget'] > 0)
                    <div class="mb-4">
                        <span class="text-lg font-semibold text-gray-700">Budget :</span>
                        <span class="ml-2 text-lg text-gray-600">${{ number_format($movieData['budget']) }}</span>
                    </div>
                @endif

                @if(isset($movieData['revenue']) && $movieData['revenue'] > 0)
                    <div class="mb-4">
                        <span class="text-lg font-semibold text-gray-700">Recettes :</span>
                        <span class="ml-2 text-lg text-gray-600">${{ number_format($movieData['revenue']) }}</span>
                    </div>
                @endif

                @if(isset($movieData['budget']) && isset($movieData['revenue']) && $movieData['budget'] > 0 && $movieData['revenue'] > 0)
                    @php
                        $profit = $movieData['revenue'] - $movieData['budget'];
                        $roi = ($profit / $movieData['budget']) * 100;
                    @endphp
                    <div class="mb-4">
                ...
                @if(isset($movieData['popularity']))
                    @php
                        $hasFinancialInfo = (isset($movieData['budget']) && $movieData['budget'] > 0) 
                                         || (isset($movieData['revenue']) && $movieData['revenue'] > 0);
                    @endphp
                    <div class="mt-6 {{ $hasFinancialInfo ? 'pt-6 border-t border-gray-200' : '' }}">
                        ...
                        <span class="ml-2 text-lg text-gray-600">{{ number_format($movieData['popularity'], 1) }}</span>
                    </div>
                @endif
            </div>
        </div>

        @if(isset($movieData['cast']) && is_array($movieData['cast']) && count($movieData['cast']) > 0)
            <div class="bg-white bg-opacity-95 backdrop-blur-sm rounded-lg shadow-xl p-8 mt-8">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                        {{ count($movieData['cast']) }} acteurs
                    </span>
                </div>
                
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                    @foreach($movieData['cast'] as $index => $actor)
                        <x-cast-card :actor="$actor" :index="$index" />
                    @endforeach
                </div>
                
                @if(count($movieData['cast']) >= 20)
        ...
        @if(isset($movieData['belongs_to_collection']) && $movieData['belongs_to_collection'])
            <div class="bg-white bg-opacity-95 backdrop-blur-sm rounded-lg shadow-xl p-8 mt-8">
                ...
                <div class="flex items-center">
                    @if(isset($movieData['belongs_to_collection']['poster_path']) && $movieData['belongs_to_collection']['poster_path'])
                        <img src="{{ \App\Helpers\ImageHelper::getPosterUrl($movieData['belongs_to_collection']['poster_path']) }}" 
                             alt="{{ $movieData['belongs_to_collection']['name'] }}" 
                             class="w-16 h-24 mr-4 object-cover rounded">
                    @endif
                    <div>
                        <h3 class="text-xl font-semibold text-gray-800">{{ $movieData['belongs_to_collection']['name'] }}</h3>
                    </div>
                </div>
            </div>
        @endif
